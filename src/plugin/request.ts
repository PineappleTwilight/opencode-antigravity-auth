...
import { cacheSignature, getCachedSignature } from "./cache.ts"
import { getKeepThinking } from "./config/index.ts"
import {
  createStreamingTransformer,
  transformSseLine,
  transformStreamingPayload,
} from "./core/streaming/index.ts"
import { defaultSignatureStore } from "./stores/signature-store.ts"
import {
...
import { sanitizeCrossModelPayloadInPlace } from "./transform/cross-model-sanitizer.ts"
import { isGemini3Model, isImageGenerationModel, buildImageGenerationConfig, applyGeminiTransforms } from "./transform/index.ts"
import {
...
import { detectErrorType } from "./recovery.ts"
import { getSessionFingerprint, buildFingerprintHeaders, type Fingerprint } from "./fingerprint.ts"
import type { GoogleSearchConfig } from "./transform/types.ts"
...
function extractTextFromContent(content: unknown): string {
  ...
  for (const block of content) {
    ...
    const part = block as AntigravityPart
    if (typeof part.text === "string") {
      return part.text
    }
    if (part.text && typeof part.text === "object" && typeof (part.text as { text?: string }).text === "string") {
      return (part.text as { text?: string }).text!
    }
  }
  return ""
}

function extractConversationSeedFromMessages(messages: AntigravityMessage[]): string {
...
function extractConversationSeedFromContents(contents: AntigravityContent[]): string {
...
function resolveConversationKey(requestPayload: AntigravityApiBody): string | undefined {
  ...
  const candidates = [
    ...
    payload.chat_id as string | undefined,
    payload.chatId as string | undefined,
    payload.sessionId as string | undefined,
    payload.session_id as string | undefined,
    (payload.metadata as Record<string, unknown> | undefined)?.conversation_id as string | undefined,
    (payload.metadata as Record<string, unknown> | undefined)?.conversationId as string | undefined,
    (payload.metadata as Record<string, unknown> | undefined)?.thread_id as string | undefined,
    (payload.metadata as Record<string, unknown> | undefined)?.threadId as string | undefined,
  ]
  ...
  const systemInstruction = payload.systemInstruction || payload.system || payload.system_instruction
  const systemSeed = extractTextFromContent(
    (systemInstruction as AntigravityContent)?.parts
    ?? systemInstruction
  )
  const messageSeed = Array.isArray(payload.messages)
    ? extractConversationSeedFromMessages(payload.messages as AntigravityMessage[])
    : Array.isArray(payload.contents)
      ? extractConversationSeedFromContents(payload.contents as AntigravityContent[])
      : ""
...
function injectDebugThinking(response: unknown, debugText: string): unknown {
  ...
  const resp = response as AntigravityApiBody

  if (Array.isArray(resp.candidates) && resp.candidates.length > 0) {
    const candidates = resp.candidates.slice() as Array<{ content?: AntigravityContent; [key: string]: unknown }>
...
function stripInjectedDebugFromParts(parts: unknown): unknown {
  ...
  return (parts as AntigravityPart[]).filter((part) => {
...
function stripInjectedDebugFromRequestPayload(payload: AntigravityApiBody): void {
  if (Array.isArray(payload.contents)) {
    payload.contents = payload.contents.map((content) => {
      ...
      if (Array.isArray(content.parts)) {
        return { ...content, parts: stripInjectedDebugFromParts(content.parts) as AntigravityPart[] }
      }

      if (Array.isArray(content.content)) {
        return { ...content, content: stripInjectedDebugFromParts(content.content) as AntigravityPart[] }
      }
  ...
  if (Array.isArray(payload.messages)) {
    payload.messages = payload.messages.map((message) => {
      ...
      if (Array.isArray(message.content)) {
        return { ...message, content: stripInjectedDebugFromParts(message.content) as AntigravityPart[] }
      }
...
function sanitizeRequestPayloadForAntigravity(payload: AntigravityApiBody): void {
  if (Array.isArray(payload.contents)) {
    const sanitizedContents = payload.contents
      .map((content) => {
        if (!content || typeof content !== "object") {
          return null
        }
        ...
        const sanitizedParts = rawParts.filter(isValidRequestPart).map((part) => {
          if (part && typeof part === "object" && part.functionCall) {
            ...
            if (!foundFirstFunctionCall) {
              ...
              if (!sig || (sig as string).length < MIN_SIGNATURE_LENGTH) {
            ...
            if (sig) {
              return { ...part, thought_signature: sig, thoughtSignature: sig } as AntigravityPart
            }
            ...
            return newPart as AntigravityPart
          }
          return part
        })
        ...
        } as AntigravityContent
      })
      .filter((content): content is AntigravityContent => content !== null)
  ...
  if (systemInstruction && typeof systemInstruction === "object" && !Array.isArray(systemInstruction)) {
    const sys = systemInstruction as AntigravityContent
...
function isGeminiToolUsePart(part: AntigravityPart): boolean {
  return !!(part && typeof part === "object" && (part.functionCall || part.tool_use || part.toolUse))
}

function isGeminiThinkingPart(part: AntigravityPart): boolean {
  return !!(
    part &&
    typeof part === "object" &&
...
function getThinkingPartText(part: AntigravityPart): string {
  if (!part || typeof part !== "object") {
    return ""
  }
...
function hasCachedMatchingSignature(part: AntigravityPart, sessionId: string): boolean {
  if (!part || typeof part !== "object") {
    return false
  }
...
function ensureThoughtSignature(part: AntigravityPart, sessionId: string): AntigravityPart {
  if (!part || typeof part !== "object") {
    return part
  }
...
function hasSignedThinkingPart(part: AntigravityPart, sessionId?: string): boolean {
  if (!part || typeof part !== "object") {
    return false
  }
...
function ensureThinkingBeforeToolUseInContents(contents: AntigravityContent[], signatureSessionKey: string): AntigravityContent[] {
  return contents.map((content) => {
    ...
    const injected: AntigravityPart = {
      thought: true,
...
function ensureMessageThinkingSignature(block: AntigravityPart, sessionId: string): AntigravityPart {
  if (!block || typeof block !== "object") {
    return block
  }
...
function hasToolUseInContents(contents: AntigravityContent[]): boolean {
...
function hasSignedThinkingInContents(contents: AntigravityContent[], sessionId?: string): boolean {
  return contents.some((content) => {
    if (!content || typeof content !== "object" || !Array.isArray(content.parts)) {
      return false
    }
...
function hasToolUseInMessages(messages: AntigravityMessage[]): boolean {
  return messages.some((message) => {
    if (!message || typeof message !== "object" || !Array.isArray(message.content)) {
      return false
    }
    return (message.content as AntigravityPart[]).some(
      (block) => block && typeof block === "object" && (block.type === "tool_use" || block.type === "tool_result"),
    )
  })
}

function hasSignedThinkingInMessages(messages: AntigravityMessage[], sessionId?: string): boolean {
  return messages.some((message) => {
    if (!message || typeof message !== "object" || !Array.isArray(message.content)) {
      return false
    }
    return (message.content as AntigravityPart[]).some((block) => hasSignedThinkingPart(block, sessionId))
  })
}

function ensureThinkingBeforeToolUseInMessages(messages: AntigravityMessage[], signatureSessionKey: string): AntigravityMessage[] {
  return messages.map((message) => {
    ...
    const blocks = message.content as AntigravityPart[]
    ...
    if (!lastThinking) {
      ...
      const thinkingText = (existingThinking?.thinking as string) || existingThinking?.text || ""
      log.debug("Injecting sentinel signature (cache miss)", { signatureSessionKey })
      const sentinelBlock: AntigravityPart = {
        type: "thinking",
    ...
    try {
      ...
      if (isWrapped) {
        ...
        if (requestRoot && typeof requestRoot === "object") {
          ...
          const nested = (requestRoot as AntigravityApiBody).request
        ...
        for (const req of requestObjects) {
          const apiReq = req as AntigravityApiBody
          // Use stable session ID for signature caching across multi-turn conversations
          apiReq.sessionId = signatureSessionKey
          stripInjectedDebugFromRequestPayload(apiReq)
          ...
          if (isClaude) {
            ...
            if (enableClaudePromptAutoCaching && apiReq.cache_control === undefined) {
              apiReq.cache_control = { type: "ephemeral" }
            }

            // Step 2: THEN inject signed thinking from cache (after stripping)
            if (isClaudeThinking && keepThinkingEnabled && Array.isArray(apiReq.contents)) {
              apiReq.contents = ensureThinkingBeforeToolUseInContents(apiReq.contents, signatureSessionKey)
            }
            if (isClaudeThinking && keepThinkingEnabled && Array.isArray(apiReq.messages)) {
              apiReq.messages = ensureThinkingBeforeToolUseInMessages(apiReq.messages, signatureSessionKey)
            }
        ...
        if (isClaudeThinking && keepThinkingEnabled && sessionId) {
          const hasToolUse = requestObjects.some((req) => {
            const apiReq = req as AntigravityApiBody
            return (Array.isArray(apiReq.contents) && hasToolUseInContents(apiReq.contents)) ||
            (Array.isArray(apiReq.messages) && hasToolUseInMessages(apiReq.messages))
          })
          const hasSignedThinking = requestObjects.some((req) => {
            const apiReq = req as AntigravityApiBody
            return (Array.isArray(apiReq.contents) && hasSignedThinkingInContents(apiReq.contents, signatureSessionKey)) ||
            (Array.isArray(apiReq.messages) && hasSignedThinkingInMessages(apiReq.messages, signatureSessionKey))
          })
        ...
        log.debug(`[ThinkingResolution] rawModel=${rawModel} resolvedModel=${effectiveModel} resolvedTier=${tierThinkingLevel ?? "none"} variantLevel=${variantConfig?.thinkingLevel ?? "none"} variantBudget=${variantConfig?.thinkingBudget ?? "none"} providerOptions.google=${JSON.stringify(((requestPayload.providerOptions as Record<string, unknown> | undefined)?.google) ?? null)} generationConfig.thinkingConfig=${JSON.stringify(rawGenerationConfig?.thinkingConfig ?? null)}`)
        ...
        const hasAssistantHistory = Array.isArray(requestPayload.contents) &&
          (requestPayload.contents as AntigravityContent[]).some((c) => c?.role === "model" || c?.role === "assistant")
        ...
        if (hasTools) {
          if (isClaude) {
            ...
            const normalizeSchema = (schema: JsonSchema | undefined) => {
              const createPlaceholderSchema = (base: JsonSchema = {}): JsonSchema => ({
                ...base,
                type: "object",
                properties: {
                  [EMPTY_SCHEMA_PLACEHOLDER_NAME]: {
            ...
            (requestPayload.tools as AntigravityTool[]).forEach((tool) => {
              const pushDeclaration = (decl: AntigravityFunctionDeclaration, source: string) => {
                const schema =
                  decl?.parameters ||
                  (decl as Record<string, unknown>).parametersJsonSchema as JsonSchema | undefined ||
                  (decl as Record<string, unknown>).input_schema as JsonSchema | undefined ||
                  (decl as Record<string, unknown>).inputSchema as JsonSchema | undefined ||
                  (tool as Record<string, unknown>).parameters as JsonSchema | undefined ||
                  (tool as Record<string, unknown>).parametersJsonSchema as JsonSchema | undefined ||
                  (tool as Record<string, unknown>).input_schema as JsonSchema | undefined ||
                  (tool as Record<string, unknown>).inputSchema as JsonSchema | undefined ||
                  (tool.function as Record<string, unknown> | undefined)?.parameters as JsonSchema | undefined ||
                  (tool.function as Record<string, unknown> | undefined)?.parametersJsonSchema as JsonSchema | undefined ||
                  (tool.function as Record<string, unknown> | undefined)?.input_schema as JsonSchema | undefined ||
                  (tool.function as Record<string, unknown> | undefined)?.inputSchema as JsonSchema | undefined ||
                  (tool.custom as Record<string, unknown> | undefined)?.parameters as JsonSchema | undefined ||
                  (tool.custom as Record<string, unknown> | undefined)?.parametersJsonSchema as JsonSchema | undefined ||
                  (tool.custom as Record<string, unknown> | undefined)?.input_schema as JsonSchema | undefined

                let name =
                  decl?.name ||
                  (tool.name as string | undefined) ||
                  ((tool.function as Record<string, unknown> | undefined)?.name as string | undefined) ||
                  ((tool.custom as Record<string, unknown> | undefined)?.name as string | undefined) ||
                ...
                const description =
                  ...
                  ((tool.function as Record<string, unknown> | undefined)?.description as string | undefined) ||
                  ((tool.custom as Record<string, unknown> | undefined)?.description as string | undefined) ||
                  ""
              ...
              if (
                ...
                pushDeclaration((tool.function ?? tool.custom ?? tool) as AntigravityFunctionDeclaration, "function/custom")
                return
              }
            ...
            const finalTools: AntigravityTool[] = []
            if (functionDeclarations.length > 0) {
              finalTools.push({ functionDeclarations })
            }
            requestPayload.tools = finalTools.concat(passthroughTools as AntigravityTool[])
          } else {
        ...
        if (isClaude && Array.isArray(requestPayload.contents)) {
          ...
          requestPayload.contents = (requestPayload.contents as AntigravityContent[]).map((content) => {
          ...
          requestPayload.contents = fixToolResponseGrouping(requestPayload.contents as AntigravityContent[])
        }
        ...
        if (Array.isArray(requestPayload.messages)) {
          requestPayload.messages = validateAndFixClaudeToolPairing(requestPayload.messages as AntigravityMessage[])
        }
        ...
        if (wrappedBody.request && typeof wrappedBody.request === 'object') {
          // Use stable session ID for signature caching across multi-turn conversations
          sessionId = signatureSessionKey;
          (wrappedBody.request as AntigravityApiBody).sessionId = signatureSessionKey
        }
...
export function buildThinkingWarmupBody(
  ...
  const updateRequest = (req: Record<string, unknown>) => {
    (req as AntigravityApiBody).contents = [{ role: "user", parts: [{ text: warmupPrompt }] }]
    delete req.tools
  ...
  if (parsed.request && typeof parsed.request === "object") {
    ...
    const nested = (parsed.request as AntigravityApiBody).request
    if (nested && typeof nested === "object") {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (errorBody?.error?.details && Array.isArray(errorBody.error.details)) {
        const retryInfo = (errorBody.error.details as Array<Record<string, unknown>>).find(
