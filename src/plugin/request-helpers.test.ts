...
import { deduplicateThinkingText, createThoughtBuffer } from "./core/streaming/transformer.ts"
...
describe("filterUnsignedThinkingBlocks", () => {
  it("filters out unsigned thinking parts", () => {
    ...
    const result = filterUnsignedThinkingBlocks(contents as any) as any
    expect(result[0].parts).toHaveLength(1)
  ...
  it("keeps signed thinking parts with valid signatures from our cache", () => {
    ...
    const result = filterUnsignedThinkingBlocks(contents as any, "session-1", getCachedSignatureFn) as any
...
describe("rewriteAntigravityPreviewAccessError", () => {
  it("returns null for non-404 status", () => {
    const body = { error: { message: "Not found" } } as AntigravityApiBody
...
describe("findOrphanedToolUseIds", () => {
  it("returns empty set when no tool_use blocks", () => {
    ...
    const result = findOrphanedToolUseIds(messages as any)
...
describe("fixClaudeToolPairing", () => {
  it("does not modify messages without tool_use", () => {
    ...
    const result = fixClaudeToolPairing(messages as any) as any
    expect(result).toEqual(messages)
  })
...
describe("validateAndFixClaudeToolPairing", () => {
  it("returns messages unchanged when no orphans", () => {
    ...
    const result = validateAndFixClaudeToolPairing(messages as any) as any
    expect(result).toEqual(messages)
  })
...
describe("cleanJSONSchemaForAntigravity", () => {
  describe("enum merging from anyOf/oneOf", () => {
    ...
    it("merges oneOf with enum values", () => {
      const schema = {
        type: "object",
        properties: {
          status: {
      ...
      expect(result.properties!.status!.enum).toEqual(["active", "pending", "inactive"])
      expect(result.properties!.status!.oneOf).toBeUndefined()
    })

    it("handles mixed const and enum", () => {
      const schema = {
        type: "object",
        properties: {
          level: {
    ...
    it("preserves other types in enum merging", () => {
      const schema = {
        type: "object",
        properties: {
          color: {
      ...
      expect(result.properties!.color!.enum).toEqual(["red"])
    })

    it("merges anyOf with nested anyOf", () => {
      const schema = {
        type: "object",
        properties: {
          data: {
      ...
      expect(result.properties!.data!.enum).toEqual(["a", "b", "c"])
      expect(result.properties!.data!.anyOf).toBeUndefined()
    })

    it("avoids duplicate enums", () => {
      const schema = {
        type: "object",
        properties: {
          format: {
      ...
      expect(result.properties!.format!.enum).toEqual(["text", "html"])
      expect(result.properties!.format!.anyOf).toBeUndefined()
    })
  })

  describe("nullable handling", () => {
    it("handles anyOf with null type", () => {
      const schema = {
        type: "object",
        properties: {
          status: {
      ...
      const result = cleanJSONSchemaForAntigravity(schema)
      expect(result.properties!.status!.enum).toEqual(["a", "b"])
      expect(result.properties!.status!.description).toContain("nullable")
      expect(result.properties!.status!.anyOf).toBeUndefined()
      expect(result.properties!.status!.type).toBe("string")
    })

    it("handles oneOf with null type", () => {
      const schema = {
        type: "object",
        properties: {
          level: {
      ...
      const result = cleanJSONSchemaForAntigravity(schema)
      expect(result.properties!.level!.description).toContain("nullable")
    })
  })
})
