...
import { SKIP_THOUGHT_SIGNATURE } from "../constants.ts"
import { GEMINI_CLI_HEADERS } from "../generated/headers.ts"
import * as config from "./config/index.ts"
import type { SignatureStore, ThoughtBuffer, StreamingCallbacks, StreamingOptions } from "./core/streaming/types.ts"
...
describe("request.ts", () => {
  ...
  describe("isGeminiToolUsePart", () => {
    it("returns true for functionCall parts", () => {
      expect(isGeminiToolUsePart({ functionCall: { name: "test", args: {} } })).toBe(true)
    })
    ...
    it("returns false for null/undefined", () => {
      expect(isGeminiToolUsePart(null as any)).toBe(false)
      expect(isGeminiToolUsePart(undefined as any)).toBe(false)
    })
  })
  ...
  describe("ensureThoughtSignature", () => {
    ...
    it("returns null/undefined inputs unchanged", () => {
      expect(ensureThoughtSignature(null as any, "key")).toBeNull()
      expect(ensureThoughtSignature(undefined as any, "key")).toBeUndefined()
    })

    it("returns non-object inputs unchanged", () => {
      expect(ensureThoughtSignature("string" as any, "key")).toBe("string")
      expect(ensureThoughtSignature(123 as any, "key")).toBe(123)
    })
  })
